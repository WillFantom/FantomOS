...
uint32_t *VirtMem_mapToCurrent(uint32_t paddr, uint8_t page)
{
    ...
    /*((page_table_entry_t *)(0xFFC00000 + ((TEMP_VADDR + (PAGE_SIZE * page)) >> 12) * 4))->page_address = (paddr >> 12);
    VirtMem_invalidateTLBE(TEMP_VADDR);
    return TEMP_VADDR + (PAGE_SIZE * page);*/
...
void VirtMem_cpyDir(page_dir_t *srcDir, page_dir_t *dstDir, bool user)
{
    ...
    for(uint32_t heap = (KERNEL_HEAP_BASE >> 22) ; heap < ((KERNEL_HEAP_BASE + KERNEL_HEAP_SIZE_MAX) >> 22) ; heap++)
    {
        ...
        if(srcDir_v->entry[heap].present)
        {
        ...
        //Debug_kprintf("Test KERNEL MAP %d\n", heap);
    }

    //Copy Dir Entries
    /*for(uint32_t e = 0 ; e < 1023 ; e++)
    {
        page_dir_entry_t *entry = &srcDir_v->entry[e];
        page_dir_entry_t *dEntry = &dstDir_v->entry[e];
        memset(dEntry, 0, sizeof(page_dir_entry_t));

        if( (entry->mode_flag == KERNEL_MODE || user) && entry->present )
        {
            kprintf("Coping DIR e %d\n", e);
            dEntry->page_address = srcDir_v->entry[e].page_address;
            dEntry->present = 0x1;
            dEntry->mode_flag = entry->mode_flag;
            dEntry->read_write_flag = entry->read_write_flag;
